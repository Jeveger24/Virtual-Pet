...
def changePicture(win):
    ...
    # These are coordinates for the pet image display in the window.
...
def buttonPress(win):
    ...
    if mouseClick != None:
        ...
        if 244.0 < x < 291.0:
            # Evaluate its y-value to determine whic button was chosen.
            y = mouseClick.getY()

            # These are the y-coordinate of the three potential buttons.
...
def alive(win):
    ...
    if day < 10:
        dayNumber = Text(Point(173, 37), day)
    else:
        dayNumber = Text(Point(176, 37), day)

    # These are ars that display updated values of the pet's stas each day.
    # Scale the ars to correspond to the pet's stat value.
    fx2 = (3/2) * hunger
...
def main():
    ...
    hungerCount = Text(Point(157, 37), "Hunger")
    hungerCount.draw(win)

    fatigueCount = Text(Point(157, 37), "Fatigue")
    fatigueCount.draw(win)

    stressCount = Text(Point(157, 37), "Stress")
    ...
    # Wait for the user to click the exit window.
    win.getMouse()
